...
inventory.get(
  ...
  async (c) => {
    ...
    // Images are served via GET /images/products/:id (public proxy to MinIO)
    // instead of /api/products/:id/image which requires auth.
...
inventory.get("/products/:id", validateUuidParam, async (c) => {
  ...
  // Use public proxy URL for image (no auth needed for <img> tags)
