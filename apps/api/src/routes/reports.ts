...
const sendEmailSchema = z.object({
  ...
  period: z.enum(["today", "week", "month", "last_month", "custom"]).default("today"),
});
 ...
 * Generates the PDF server-side and sends it as attachment via Resend.
 */
reports.post(
  "/reports/send-email",
  ...
  async (c) => {
    ...
    // Generate the PDF based on report type
    // Reuse the existing PDF export logic by calling the internal functions
