  ...
  <div class="flex min-h-screen flex-col bg-white transition-colors dark:bg-black">
    ...
    <nav class="fixed bottom-0 w-full border-t border-gray-100 bg-white pb-[env(safe-area-inset-bottom)] pt-3 z-40 shadow-[0_-4px_20px_rgba(0,0,0,0.04)] dark:border-white/8 dark:bg-black will-change-transform">
      <div class="flex items-center justify-around px-4">
        ...
        <!-- WhatsApp — always shown in slot 4; active when business has a number -->
        <component
          :is="whatsappLink ? 'a' : 'span'"
          v-bind="whatsappLink ? { href: whatsappLink, target: '_blank', rel: 'noopener noreferrer' } : {}"
          class="flex flex-col items-center gap-0.5 transition-colors"
          :class="whatsappLink ? 'text-[#25D366]' : 'text-gray-300 dark:text-gray-600 cursor-default'"
          aria-label="WhatsApp"
        >
