  ...
  <div class="flex min-h-screen flex-col bg-white transition-colors dark:bg-gray-950">
    ...
    <nav class="fixed bottom-0 w-full border-t border-gray-100 bg-white pb-[env(safe-area-inset-bottom)] pt-3 z-40 shadow-[0_-4px_20px_rgba(0,0,0,0.04)] dark:border-gray-800 dark:bg-gray-950">
      <div class="flex items-center justify-around px-4">
        ...
        <!-- WhatsApp (center, elevated, green) -->
        <a
          v-if="whatsappLink"
          :href="whatsappLink"
          target="_blank"
          rel="noopener noreferrer"
          class="relative -mt-7 flex h-[56px] w-[56px] items-center justify-center rounded-full border-4 border-white bg-[#25D366] text-white shadow-[0_6px_20px_rgba(37,211,102,0.35)] transition-transform active:scale-95 dark:border-gray-950"
          aria-label="WhatsApp"
        >
        ...
        <!-- Fallback if no WhatsApp: empty space to keep layout balanced -->
        <div v-else class="w-[56px]" />

        <!-- Carrito -->
        <NuxtLink
          to="/tienda/cart"
          class="relative flex flex-col items-center gap-0.5 transition-colors"
          :class="isActive('/tienda/cart') ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-500'"
        >
          <div class="relative">
            <ShoppingCart :size="22" />
            <span
              v-if="itemCount > 0"
              class="absolute -right-2 -top-1.5 flex h-4 min-w-[16px] items-center justify-center rounded-full bg-red-500 px-1 text-[9px] font-bold leading-none text-white"
            >
