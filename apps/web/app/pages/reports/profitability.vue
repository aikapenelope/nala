...
const narrative = ref("Pan Campesino lidera con score 92 (alto margen + alta rotación). Queso Blanco tiene margen bajo (33%) pero buena rotación.");
const products = ref([
      ...
      <table class="w-full text-left text-sm">
        <thead class="border-b bg-gray-50">
          <tr>
            ...
            <th class="px-5 py-3 text-right font-medium text-gray-500">Margen</th>
            <th class="px-5 py-3 text-right font-medium text-gray-500">Rotación</th>
            <th class="px-5 py-3 text-right font-medium text-gray-500">Contribución</th>
            <th class="px-5 py-3 text-right font-medium text-gray-500">Score</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-100">
          <tr v-for="p in products" :key="p.name">
            ...
            <td class="px-5 py-3 text-right text-gray-700">{{ p.contribution }}%</td>
            <td class="px-5 py-3 text-right font-semibold" :class="p.score >= 70 ? 'text-green-600' : p.score >= 40 ? 'text-yellow-600' : 'text-red-600'">{{ p.score }}</td>
          </tr>
        </tbody>
      </table>
    </div>
